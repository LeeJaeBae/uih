  ...
  async generate(file: UIHFile): Promise<string> {
    ...
    if (!layout) {
      throw new UIHMissingBlockError(
        "Layout",
        "Svelte code generation requires a layout block to define the component structure"
      );
    }
...
`;
    ...
    try {
      ...
      const prettierPluginSvelte = await import("prettier-plugin-svelte");
      const formatted = await prettier.default.format(code.trim(), {
        ...
        trailingComma: "es5",
        printWidth: 80,
        plugins: [prettierPluginSvelte.default],
      });
      return formatted;
    } catch (error) {
...
${indentStr}</${svelteComponent}>`;
  ...
  private mapToSvelteComponent(name: string): string {
    // Map component names to Svelte/HTML equivalents (HTML tag names)
