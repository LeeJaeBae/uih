...
2. **Component Support Expanded**: Registry now includes shadcn/ui components + pure HTML elements (Div, H1-H6, Form, A, Img, etc.)
3. **Type Safety**: Multiple `any` types in codegen registry (registry.ts:4,9,14,19)
4. **Parser Limitations**: Nested component children in layout blocks have limited support (works for single Text nodes only)
